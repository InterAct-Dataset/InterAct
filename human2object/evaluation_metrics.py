...
def compute_metrics(ori_markers_gt, ori_markers_pred, obj_verts, actual_len, use_joints24=False):
    # verts_gt: T X Nv X 3 
    # jpos_gt: T X J X 3
    # gt_trans: T X 3
    ...
    rhand_idx = [60, 43, 44, 47, 62, 46, 51, 57]
    lhand_idx = [10, 11, 14, 31, 13, 17, 23, 28, 27]

    ori_markers_gt = ori_markers_gt[:actual_len]
    ori_markers_pred = ori_markers_pred[:actual_len]
    obj_verts = obj_verts[:actual_len]
     
    # Calculate global hand joint position error 

    lhand_jpos_pred = ori_markers_pred[:, lhand_idx, :].detach().cpu().numpy() 
    rhand_jpos_pred = ori_markers_pred[:, rhand_idx, :].detach().cpu().numpy() 
    lhand_jpos_gt = ori_markers_gt[:, lhand_idx, :].detach().cpu().numpy()
    rhand_jpos_gt = ori_markers_gt[:, rhand_idx, :].detach().cpu().numpy() 
    lhand_jpe = np.linalg.norm(lhand_jpos_pred - lhand_jpos_gt, axis=-1).mean()
    rhand_jpe = np.linalg.norm(rhand_jpos_pred - rhand_jpos_gt, axis=-1).mean()
    hand_jpe = (lhand_jpe+rhand_jpe)/2.0 

    # Calculate MPVPE 
    verts_pred = ori_markers_pred 
    verts_gt = ori_markers_gt 
    verts_pred = verts_pred.detach().cpu().numpy()
    verts_gt = verts_gt.detach().cpu().numpy()
    mpvpe = np.linalg.norm(verts_pred - verts_gt, axis=-1).mean() 
    ...
    gt_lhand_jnt = ori_markers_gt[:, lhand_idx, :] # T X v X 3 
    gt_rhand_jnt = ori_markers_gt[:, rhand_idx, :] # T X v X 3 

    gt_lhand2obj_dist = torch.sqrt(((gt_lhand_jnt[..., None, :].repeat(1, 1, num_obj_verts, 1) - obj_verts[:,None,...].to(gt_lhand_jnt.device))**2).sum(dim=-1)) # T X N  
    gt_rhand2obj_dist = torch.sqrt(((gt_rhand_jnt[..., None, :].repeat(1, 1, num_obj_verts, 1) - obj_verts[:,None,...].to(gt_rhand_jnt.device))**2).sum(dim=-1)) # T X N  
    
    gt_lhand2obj_dist = gt_lhand2obj_dist.min(dim=-1)[0]
    gt_rhand2obj_dist = gt_rhand2obj_dist.min(dim=-1)[0]
    ...
    lhand_jnt = ori_markers_pred[:, lhand_idx, :] # T X 3 
    rhand_jnt = ori_markers_pred[:, rhand_idx, :] # T X 3 

    lhand2obj_dist = torch.sqrt(((lhand_jnt[..., None, :].repeat(1, 1, num_obj_verts, 1) - obj_verts[:,None,...].to(lhand_jnt.device))**2).sum(dim=-1)) # T X N  
    rhand2obj_dist = torch.sqrt(((rhand_jnt[..., None, :].repeat(1, 1, num_obj_verts, 1) - obj_verts[:,None,...].to(rhand_jnt.device))**2).sum(dim=-1)) # T X N  
    
    lhand2obj_dist = lhand2obj_dist.min(dim=-1)[0]
    rhand2obj_dist = rhand2obj_dist.min(dim=-1)[0]
    ...
    return lhand_jpe, rhand_jpe, hand_jpe, mpvpe, gt_contact_dist, contact_dist, \
    contact_precision, contact_recall, contact_acc, contact_f1_score  
 



# compute_metrics_obj(human_verts_list, human_faces_list, \
#                             gt_obj_trans_list, pred_obj_trans_list, gt_obj_rot_list, pred_obj_rot_list, \
#                             gt_obj_verts_list, pred_obj_verts_list, obj_faces_list, actual_len_list, use_joints24=False)
def compute_metrics_obj(ori_markers, gt_trans, pred_trans, gt_rot_mat, pred_rot_mat, gt_obj_verts, pred_obj_verts, actual_len, use_joints24=False):
    # verts_gt: T X Nv X 3 
    # jpos_gt: T X J X 3
    # gt_trans: T X 3
    # gt_rot_mat: T X 22 X 3 X 3 
    ...
    ori_markers = ori_markers[:actual_len]

     


    # Calculate MPVPE 
    verts_pred = pred_obj_verts 
    verts_gt = gt_obj_verts 
    verts_pred = verts_pred.detach().cpu().numpy()
    verts_gt = verts_gt.detach().cpu().numpy()
    mpvpe = np.linalg.norm(verts_pred - verts_gt, axis=2).mean()



    # Caculate translation error 
    trans_err = np.linalg.norm(pred_trans.detach().cpu().numpy() - gt_trans.detach().cpu().numpy(), axis=1).mean()
    
    # Calculate rotation error
    rot_mat_pred = pred_rot_mat.detach().cpu().numpy() # Only evaluate for root rotation 
    rot_mat_gt = gt_rot_mat.detach().cpu().numpy()
    ...
    gt_lhand_jnt = ori_markers[:, lhand_idx, :] # T X 3 
    gt_rhand_jnt = ori_markers[:, rhand_idx, :] # T X 3 

    gt_lhand2obj_dist = torch.sqrt(((gt_lhand_jnt[..., None, :].repeat(1,1, num_obj_verts, 1) - gt_obj_verts[:, None, ...].to(gt_lhand_jnt.device))**2).sum(dim=-1)) # T X N  
    gt_rhand2obj_dist = torch.sqrt(((gt_rhand_jnt[..., None, :].repeat(1,1, num_obj_verts, 1) - gt_obj_verts[:, None, ...].to(gt_rhand_jnt.device))**2).sum(dim=-1)) # T X N  
    ...
    lhand_jnt = ori_markers[:, lhand_idx, :] # T X 3 
    rhand_jnt = ori_markers[:, rhand_idx, :] # T X 3 

    lhand2obj_dist = torch.sqrt(((lhand_jnt[..., None, :].repeat(1,1, num_obj_verts, 1) - pred_obj_verts[:, None, ...].to(lhand_jnt.device))**2).sum(dim=-1)) # T X N  
    rhand2obj_dist = torch.sqrt(((rhand_jnt[..., None, :].repeat(1,1, num_obj_verts, 1) - pred_obj_verts[:, None, ...].to(rhand_jnt.device))**2).sum(dim=-1)) # T X N  
    ...
    return mpvpe * 100, rot_dist, trans_err, gt_contact_dist, contact_dist, \
    contact_precision, contact_recall, contact_acc, contact_f1_score  
