...
class CondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.motion_encoder = nn.Sequential(
            nn.Linear(in_features=553, out_features=231),
            nn.ReLU(),
            nn.Linear(in_features=231, out_features=128),
            )
        # Input: (BS*T) X 3 X N 
        ...
        obj_feats_dim = 256 
        motion_feats = 128
        d_input_feats = d_feats+obj_feats_dim+motion_feats
    ...
    def sample(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None):
        ...
        x_cond =  torch.cat((self.bps_encoder(ori_x_cond[...,:1024*3]),self.motion_encoder(ori_x_cond[...,1024*3:])),dim=-1) # BS X T X (256+128) 
