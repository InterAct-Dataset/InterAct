...
from data.dataset_smpl import Dataset
...
from render.mesh_viz import visualize_marker_obj
...
class LitInteraction(pl.LightningModule):
    def __init__(self, args):
        ...
        smpl_male = SMPL_Layer(center_idx=0, gender='male', num_betas=10,
                                model_root=str("../data/models"), hands=True).to(device)
    ...
    def forward_backward(self, batch, cond):
        ...
        marker_pred, obj_pred = torch.split(pred.squeeze(1).permute(2, 0, 1).contiguous(), 77*3, dim=2)
        marker_gt, obj_gt = torch.split(gt.squeeze(1).permute(2, 0, 1).contiguous(), 77*3, dim=2)
        ...
        loss_body_nonrot_past = self.l2(marker_pred[:self.args.past_len, :, :], marker_gt[:self.args.past_len, :, :])
        ...
        loss_body_nonrot_v_past = self.l2(marker_pred[1:self.args.past_len+1, :, :]-marker_pred[:self.args.past_len, :, :], marker_gt[1:self.args.past_len+1, :, :]-marker_gt[1:self.args.past_len+1, :, :]) +\
                                  self.l2(marker_pred[1:self.args.past_len, :, :]-marker_pred[:self.args.past_len-1, :, :], marker_pred[2:self.args.past_len+1, :, :]-marker_pred[1:self.args.past_len, :, :])
        ...
        loss_body_nonrot_future = self.l2(marker_pred[self.args.past_len:, :, :], marker_gt[self.args.past_len:, :, :])
        ...
        loss_body_nonrot_v_future = self.l2(marker_pred[self.args.past_len:, :, :]-marker_pred[self.args.past_len-1:-1, :, :], marker_gt[self.args.past_len:, :, :]-marker_gt[self.args.past_len:, :, :]) +\
                                    self.l2(marker_pred[self.args.past_len-1:-2, :, :]-marker_pred[self.args.past_len:-1, :, :], marker_pred[self.args.past_len:-1, :, :]-marker_pred[self.args.past_len+1:, :, :])
        ...
        return loss, marker_pred, obj_pred, marker_gt, obj_gt
    ...
    def calc_val_loss(self, marker_pred, marker_gt, obj_pred, obj_gt, batch):
        loss_dict = dict()
        weighted_loss_dict = dict()

        T, B, nJ = marker_pred.shape
        obj_rot = rotvec_to_rotmat(obj_pred[:, :, :-3]).view(T, B, 9)
        obj_rot_gt = rotvec_to_rotmat(obj_gt[:, :, :-3]).view(T, B, 9)

        loss_body_nonrot_past = torch.nn.MSELoss(reduction='mean')(marker_pred[:self.args.past_len, :, :], marker_gt[:self.args.past_len, :, :])
        ...
        loss_body_nonrot_v_past = torch.nn.MSELoss(reduction='mean')(marker_pred[1:self.args.past_len+1, :, :]-marker_pred[:self.args.past_len, :, :], marker_gt[1:self.args.past_len+1, :, :]-marker_gt[:self.args.past_len, :, :])
        ...
        loss_body_nonrot_future = torch.nn.MSELoss(reduction='mean')(marker_pred[self.args.past_len:, :, :], marker_gt[self.args.past_len:, :, :])
        ...
        loss_body_nonrot_v_future = torch.nn.MSELoss(reduction='mean')(marker_pred[self.args.past_len:, :, :]-marker_pred[self.args.past_len-1:-1, :, :], marker_gt[self.args.past_len:, :, :]-marker_gt[self.args.past_len-1:-1, :, :])
    ...
    def calc_loss(self, marker_pred, marker_gt, obj_pred, obj_gt, batch):
        loss_dict = dict()
        weighted_loss_dict = dict()

        T, B, nJ = marker_gt.shape
        ...
        marker_gt = marker_gt.unsqueeze(0).repeat(self.args.diverse_samples, 1, 1, 1)
        ...
        loss_body_nonrot_v_future = torch.nn.MSELoss(reduction='mean')(marker_pred[:, self.args.past_len+1:, :, :]-marker_pred[:, self.args.past_len:-1, :, :], marker_gt[:, self.args.past_len+1:, :, :]-marker_gt[:, self.args.past_len:-1, :, :])
        ...
        loss_body_nonrot_past_min = torch.nn.MSELoss(reduction='none')(marker_pred[:, :self.args.past_len, :, :], marker_gt[:, :self.args.past_len, :, :]).mean(dim=[1, 3]).min(dim=0)[0].mean()
        ...
        loss_body_nonrot_v_past_min = torch.nn.MSELoss(reduction='none')(marker_pred[:, 1:self.args.past_len+1, :, :]-marker_pred[:, :self.args.past_len, :, :], marker_gt[:, 1:self.args.past_len+1, :, :]-marker_gt[:, :self.args.past_len, :, :]).mean(dim=[1, 3]).min(dim=0)[0].mean()
        ...
        loss_body_nonrot_future_min = torch.nn.MSELoss(reduction='none')(marker_pred[:, self.args.past_len:, :, :], marker_gt[:, self.args.past_len:, :, :]).mean(dim=[1, 3]).min(dim=0)[0].mean()
        ...
        loss_body_nonrot_v_future_min = torch.nn.MSELoss(reduction='none')(marker_pred[:, self.args.past_len+1:, :, :]-marker_pred[:, self.args.past_len:-1, :, :], marker_gt[:, self.args.past_len+1:, :, :]-marker_gt[:, self.args.past_len:-1, :, :]).mean(dim=[1, 3]).min(dim=0)[0].mean()
    ...
    def _common_step(self, batch, batch_idx, mode):
        ...
        if mode == 'train':
            loss, marker_pred, obj_pred, marker_gt, obj_gt = self.forward_backward(gt, model_kwargs)
            return loss
        elif mode == 'valid':
            ...
            marker_pred, obj_pred = torch.split(sample.squeeze(1).permute(2, 0, 1).contiguous(), 77*3, dim=2)
            marker_gt, obj_gt = torch.split(gt.squeeze(1).permute(2, 0, 1).contiguous(), 77*3, dim=2)
            T, B, _ = marker_pred[:, :, :-3].shape
            ...
            loss, loss_dict, weighted_loss_dict = self.calc_val_loss(marker_pred, marker_gt, obj_pred, obj_gt, batch=batch)

            render_interval = 100
            if (batch_idx % render_interval == 0) and (((self.current_epoch % self.args.render_epoch) == self.args.render_epoch - 1) or self.args.debug):
                self.visualize(marker_pred, obj_pred, marker_gt, obj_gt, batch, batch_idx, mode, 0)
            return loss, loss_dict, weighted_loss_dict
            ...
            model_kwargs['y']['inpainting_mask'][:, :, :, self.args.past_len:] = False  # do inpainting in those frames
            sample_fn = self.diffusion.p_sample_loop
            marker_gt, obj_gt = torch.split(gt.squeeze(1).permute(2, 0, 1).contiguous(), self.args.smpl_dim+3, dim=2)
            idx_pad = list(range(self.args.past_len)) + [self.args.past_len - 1] * self.args.future_len
            # marker_gt = torch.cat([marker_gt[:, :, :-3], hand_pose, marker_gt[:, :, -3:]], dim=2)
            marker_preds, obj_preds = [], []
            T, B, _ = marker_gt[:, :, :-3].shape
            obj_gt = torch.cat([obj_rot_gt, obj_gt[:, :, -3:]], dim=2)
            for idx in range(self.args.diverse_samples):
                sample = sample_fn(self.model, gt.shape, clip_denoised=False, model_kwargs=model_kwargs)
                marker_pred, obj_pred = torch.split(sample.squeeze(1).permute(2, 0, 1).contiguous(), self.args.smpl_dim+3, dim=2)
                obj_rot = matrix_to_axis_angle(rotation_6d_to_matrix(obj_pred[:, :, :-3].view(T, B, -1, 6))).view(T, B, -1)
                obj_pred = torch.cat([obj_rot, obj_pred[:, :, -3:]], dim=2)
                marker_preds.append(marker_pred.unsqueeze(0))
                obj_preds.append(obj_pred.unsqueeze(0))
            marker_preds = torch.cat(marker_preds, dim=0)
            obj_preds = torch.cat(obj_preds, dim=0)
            loss, loss_dict, weighted_loss_dict = self.calc_loss(marker_preds, marker_gt, obj_preds, obj_gt, batch=batch)

            render_interval = 100
            if (batch_idx % render_interval == 0):
                for idx in range(self.args.diverse_samples):
                    if idx == 0:
                        self.visualize(marker_preds[idx], obj_preds[idx], marker_gt, obj_gt, batch, batch_idx, mode, idx)
                    else:
                        self.visualize(marker_preds[idx], obj_preds[idx], None, None, batch, batch_idx, mode, idx)
            return loss, loss_dict, weighted_loss_dict

    def visualize(self, marker_pred, obj_pred, marker_gt, obj_gt, batch, batch_idx, mode, idx):
        with torch.no_grad():
            marker = marker_pred.detach().clone()
            ...
            if marker_gt is not None and obj_gt is not None:
                marker_gt = marker_gt.detach().clone()
                ...
                # verts_gt, jtr_gt, _, _ = smpl(marker_gt[:, 0, :-3], 
                #                         th_betas=torch.cat([record['smplfit_params']['betas'][0:1] for record in batch['frames']], dim=0), 
                #                         th_trans=marker_gt[:, 0, -3:])
            ...
            for t, record in (enumerate(batch['frames'])):
                ...
                if batch['obj_path'][0] in train_dataset.obj_dict.keys():                  
                    mesh_obj_p= train_dataset.obj_dict[batch['obj_path'][0]][1].copy()
                    mesh_obj_v = train_dataset.obj_dict[batch['obj_path'][0]][1].copy()
                    mesh_faces = train_dataset.obj_dict[batch['obj_path'][0]][2]
                elif batch['obj_path'][0] in test_dataset.obj_dict.keys():                  
                    mesh_obj_p= test_dataset.obj_dict[batch['obj_path'][0]][1].copy()
                    mesh_obj_v = test_dataset.obj_dict[batch['obj_path'][0]][1].copy()
                    mesh_faces = test_dataset.obj_dict[batch['obj_path'][0]][2]
                else:
                    raise Exception('no obj info')
                mesh_obj_v=np.array(mesh_obj_v)
                # center the meshes
                mesh_obj_p=np.array(mesh_obj_p)
                ...
                mesh_obj_p = np.matmul(mesh_obj_p, rot.T) + trans
                obj_verts.append(mesh_obj_p)
            ...
            m1 = visualize_marker_obj(np.array(obj_verts), np.array(mesh_faces), pcd = marker[:,0], past_len=self.args.past_len, save_path=rend_video_path, sample_rate=self.args.sample_rate)
            if marker_gt is not None and obj_gt is not None:
                m2 = visualize_marker_obj(np.array(obj_verts_gt),np.array(mesh_faces), pcd = marker_gt[:,0], past_len=self.args.past_len, save_path=rend_gt_video_path, sample_rate=self.args.sample_rate)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--weight_smplx_nonrot", type=float, default=2.0)
