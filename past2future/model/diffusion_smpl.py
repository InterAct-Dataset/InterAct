...
class MDM(nn.Module):
    def __init__(self, args):
        ...
        self.bodyEmbedding = nn.Linear(77*3, num_channels)
        ...
        self.finalLinear = nn.Linear(num_channels, 77*3+6)
        self.bodyFinalLinear = nn.Linear(num_channels, 77*3)
    ...
    def _get_embeddings(self, data, device=None):
        if device:
            markers = torch.cat([frame['markers'].unsqueeze(0) for frame in data['frames']], dim=0).float().to(device) # TxBxDb
            ...
            obj_points = data['obj_verts'][:, :, :3].float().to(device)  # TxBxPx7
        else:
            markers = torch.cat([frame['markers'].unsqueeze(0) for frame in data['frames']], dim=0).float()
            ...
            obj_points = data['obj_verts'][:, :, :3].float()

        T, B, _, _ = markers.shape
        markers = markers.view(T, B, -1)
        pc_embedding = torch.cat([obj_points, obj_points.norm(dim=2, keepdim=True)], dim=2).unsqueeze(0).contiguous()
        ...
        gt = torch.cat([markers, obj_angles, obj_trans], dim=2)
        body, obj = markers, torch.cat([obj_angles, obj_trans], dim=2)
    ...
    def _decode(self, x, time_embedding, y=None):
        body, obj = torch.split(x, 77*3, dim=2)
