...
def metrics(obj_pred, marker, obj_gt, marker_gt, faces, obj_points):
    ...
    T, B,_ = marker_gt.shape# N_jtr 52?
    
    marker_gt=marker_gt.view(T,B,-1,3)
    marker=marker.view(T,B,-1,3)
    ...
    # global mpjpe
    global_mpjpe = (marker - marker_gt).norm(dim=3).mean(dim=2).mean(dim=0)

    # align pelvis
    pelvis = marker[:,:,33:34,:]
    pelvis_gt = marker_gt[:,:,33:34,:]
   

    
    marker = marker - pelvis
    marker_gt = marker_gt - pelvis_gt
    # local mpjpe
    local_mpjpe = (marker - marker_gt).norm(dim=3).mean(dim=2).mean(dim=0)
...
def sample_once_proj(batch):
    with torch.no_grad():
        ...
        #model_kwargs['y']['obj_points'] = batch['obj_verts'][:, :, :3].float().to(device)
...
def sample_once(batch):
    with torch.no_grad():
        ...
        #                        th_trans=body_batch[:, -3:])
        obj_pred = torch.cat([obj_rot, obj_pred[:, :, -3:]], dim=2)
    return obj_pred, marker_pred,  marker_pred.view(T, B, -1, 3)[:, :, 33, :]

def smooth(obj, marker, pelvis):
    obj[-args.future_len:] = obj[-args.future_len:] + (2 * obj[-args.future_len-1] - obj[-args.future_len-2] - obj[-args.future_len])
    marker[-args.future_len:] = marker[-args.future_len:] + (2 * marker[-args.future_len-1] - marker[-args.future_len-2] - marker[-args.future_len])
    pelvis[-args.future_len:] = pelvis[-args.future_len:] + (2 * pelvis[-args.future_len-1] - pelvis[-args.future_len-2] - pelvis[-args.future_len])
    return obj, marker, pelvis
...
def sample(name):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            obj_gt, marker_gt, faces = get_gt(batch)
            
            for j in range(args.diverse_samples):
                new_batch = deepcopy(batch)
                obj, marker, pelvis = sample_func(new_batch)
                
                metric = metrics(obj[args.past_len:], marker[args.past_len:], obj_gt[args.past_len:],marker_gt[args.past_len:], faces, batch['obj_verts'][:, :, :3].float().to(device))
                ...
                obj, marker, pelvis = smooth(obj, marker, pelvis)
                if i % args.render_epoch == 0:
                    visualize(batch, i, obj[:, 0], marker[:, 0], faces, name)
...
def visualize(batch, j, obj, marker, faces, name):
    ...
    for t in range(obj.shape[0]):
        ...
        if batch['obj_path'][0] in test_dataset.obj_dict.keys():                  
    ...
    pcd=torch.from_numpy(marker)
    m1 = visualize_marker_obj(np.array(obj_verts), np.array(mesh_faces),pcd =pcd , past_len=args.past_len, save_path=rend_video_path, sample_rate=args.sample_rate)
