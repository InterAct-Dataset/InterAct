...
def eval_t2hoi(val_loader, motion_model, motion_diffusion, textencoder,motionencoder,std_enc,mean_enc,repeat_id):
    ...
    MAX_REPEAT=10# insert
    motion_multimodality=[]# insert
    mutlimodality_select_times=5 # insert
    ...
    for batch_idx,batch in enumerate(val_loader):
        ...
        model_kwargs['y']['action'] = model_kwargs['y']['action'].to(dist_util.dev())
        ...
        vertices = obj_points.reshape(-1,obj_points.shape[2],3).float()

        angle, trans = motions_obj[..., :3].reshape(-1,3).float(), motions_obj[..., 3:].reshape(-1,3).float()

        rot = Rotation.from_rotvec(angle.cpu().numpy()).as_matrix()
        torch_rot = torch.from_numpy(rot).float()
        ...
        gt_motions = torch.cat((motions.cuda(),bps_time_nonrot.reshape(motions.shape[0], motions.shape[1],128*3).cuda()),dim=-1)
        ...
        sample = sample_fn(
            motion_model,
            (args.batch_size, motion_model.njoints + 6, motion_model.nfeats, gt_motions.shape[1]),    # + 6 object pose
            clip_denoised=False,
        ...
        # sample = motions_copy
        sample = val_loader.dataset.t2m_dataset.inv_transform(sample.cpu().permute(0, 2, 3, 1)).squeeze().float()
        # sample = sample.cpu().permute(0, 2, 3, 1).squeeze().float()
        sample_obj = sample[..., 553:559].float()
        angle, trans = sample_obj[..., :3].reshape(-1,3), sample_obj[..., 3:].reshape(-1,3)
        ...
        bps_time = bps_torch.encode(x=obj_points, \
            feature_type=['deltas'], \
            custom_basis=bps_obj_times)['deltas'] # T X N X 3 
        pred_motions = torch.cat((sample.cuda(),bps_time.reshape(sample.shape[0], sample.shape[1],128*3).cuda()),dim=-1)
        ...
        if batch_idx<mm_batch_num:
            ...
            for _ in range(MAX_REPEAT):
                ## insert TODO ，同样的也是和前面一样 generate 出 943 维度的motion向量，然后encode
                sample = sample_fn(
                    motion_model,
                    (args.batch_size, motion_model.njoints + 6, motion_model.nfeats,  gt_motions.shape[1]),    # + 6 object pose
                    clip_denoised=False,
    ...
    # sbert_feature_np=torch.cat(sbert_f_list, dim=0).cpu().numpy()
    # text_feature_np=torch.cat(text_f_list, dim=0).cpu().numpy()
    # SAVE_ROOT='/home/exouser/InterAct/evaluation/InterAct-HOI-Diff'
    # np.save(os.path.join(SAVE_ROOT,"sbert_embedding.npy"),sbert_feature_np)
    # np.save(os.path.join(SAVE_ROOT,"motion_embedding.npy"),motion_annotation_np)
    # np.save(os.path.join(SAVE_ROOT,"text_embedding.npy"),text_feature_np) # Only Once
    ...
    msg = "--> \t Eva. Re %d:, FID. %.4f, Diversity Real. %.4f, Diversity. %.4f, mae. %.4f" % \
          (repeat_id, fid, diversity_real, diversity, l1_dist)
    # logger.info(msg)
    print(msg)
    return fid, diversity, l1_dist
...
if __name__ == "__main__":
    ...
    # bps 
    bps_torch = bps_torch()
    bps_obj = np.load("./OpenTMA/bps_basis_set_128_1.npy")
    ...
    ## nfeats 559: motion dimension, 128*3: bps dimension
    motionencoder=ActorAgnosticEncoder(nfeats=559+128*3,vae=True,latent_dim=256,ff_size=1024,
                                    num_layers=4).cuda()
    ...
    f = open(f'./sample/{dataset}_{name}_{niter}_correct_eval.txt', 'w')
    print('final result:')
