    ...
    def __init__(
        ...
        **kwargs,
        ...
        for DataSet in ['behave', 'intercap', 'neuraldome', 'grab', 'chairs', 'omomo', 'imhd']:
            ...
            id_list_part=[[DataSet,e.strip()] for e in names_text_list]
        ...
        for i, dataset_plus_id in enumerator:
            DataSet_name=dataset_plus_id[0]
            id_name=dataset_plus_id[1]
            name=DataSet_name+'_'+id_name
            if count > maxdata:
                break
            try:
                motion_h = np.load(pjoin(motion_dir, DataSet_name,'sequences',id_name,  "motion.npy"))
                motion_bps = np.load(pjoin(motion_dir, DataSet_name,'sequences',id_name,  "bps_time_nonrot.npy"))
                ...
                with cs.open(pjoin(motion_dir, DataSet_name,'sequences',id_name,  "text.txt")) as f:
                    for line in f.readlines():
                        text_dict = {}
                        line_split = line.strip().split("#")
                        caption = line_split[0]
