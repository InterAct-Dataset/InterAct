...
from .InterAct import InterActDataModule

def get_mean_std(phase, cfg, dataset_name):

    if dataset_name=='interact':
        split_root='./OpenTMA/dataset_split'
        if phase in ["val",'test']:
            std=np.load(pjoin(split_root,'std_test.npy'))
            mean=np.load(pjoin(split_root,'mean_test.npy'))
        else:
            std=np.load(pjoin(split_root,'std_train.npy'))
            mean=np.load(pjoin(split_root,'mean_train.npy'))
        return mean,std
...
def get_WordVectorizer(cfg, phase, dataset_name):
    ...
    if phase not in ["text_only"]:
        if dataset_name.lower() in ["unimocap", "motionx", "humanml3d", "kit",'interact']:
...
def get_collate_fn(name, cfg, phase="train"):
    if name.lower() in ['interact']:
        return tma_collate_interact
...
def get_datasets(cfg, logger=None, phase="train"):
    ...
    for dataset_name in dataset_names:
        if dataset_name.lower() in ["unimocap", "humanml3d", "kit",'interact']:
            data_root = eval(f"cfg.DATASET.{dataset_name.upper()}.ROOT")
            ...
            elif dataset_name.lower()=='interact':
                dataset = dataset_module_map[dataset_name.lower()](
                    cfg=cfg,
                    batch_size=cfg.TRAIN.BATCH_SIZE,
                    num_workers=cfg.TRAIN.NUM_WORKERS,
