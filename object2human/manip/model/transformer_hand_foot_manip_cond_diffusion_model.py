...
class CondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        obj_feats_dim = 256 
        d_input_feats = d_feats+obj_feats_dim 
    ...
    def sample(self, x_start, ori_x_cond, cond_mask=None, padding_mask=None):
        ...
        x_cond = self.bps_encoder(ori_x_cond) # BS X T X (256) 
    ...
    def p_losses(self, x_start, x_cond, t, noise=None, padding_mask=None):
        ...
        if padding_mask is not None:
            loss = self.loss_fn(model_out[...,:231], target[...,:231], reduction = 'none') * padding_mask[:, 0, 1:][:, :, None]
        else:
            loss = self.loss_fn(model_out[...,:231], target[...,:231], reduction = 'none') # BS X T X D 
