...
class Trainer(object):
    ...
    def train(self):
        ...
        for idx in range(init_step, self.train_num_steps):
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                ori_data_cond = data_dict['obj_bps'].cuda() 
                cond_mask = None
            ...
            if self.step != 0 and self.step % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    cond_mask = None
                    ori_data_cond = val_data_dict['obj_bps'].cuda()
        ...
        for s_idx, val_data_dict in enumerate(test_loader):
            ...
            for sample_idx in range(num_samples_per_seq):
                ...
                for seq_idx in range(num_seq):
                    ...
                    gt_human_markers_list, \
                        obj_verts_list, actual_len_list = \
                        self.gen_vis_res_gt(val_data_dict['motion'].cuda()[seq_idx:seq_idx+1], val_data_dict, \
                    ...
                    lhand_jpe, rhand_jpe, hand_jpe, mpvpe, \
                            ...
                            compute_metrics(gt_human_markers_list, pred_human_markers_list, \
                            obj_verts_list, actual_len_list, use_joints24=False)
    ...
    def gen_vis_res_gt(self, all_res_list, data_dict, step, vis_gt=False, vis_tag=None, \
        ...
        normalized_global_markers = all_res_list[:, :, :num_markers*3].reshape(num_seq, -1, num_markers, 3)
    
        global_markers = self.ds.de_normalize_markers_min_max(normalized_global_markers.reshape(-1, num_markers, 3))
        global_markers = global_markers.reshape(num_seq, -1, num_markers, 3) # N X T X 77 X 3 
        ...
        for idx in range(num_seq):
            curr_global_markers = global_markers[idx] # T X 77 X 3 
            ...
            if selected_seq_idx is None:
                ...
                curr_obj_angles = data_dict['obj_angles'][selected_seq_idx]
            ...
            # human_trans_list.append(root_trans[start_t_idx:]) 
            # human_jnts_list.append(mesh_jnts[start_t_idx:])
            # human_verts_list.append(mesh_verts[start_t_idx:])
            # human_faces_list.append(mesh_faces)
            # human_rot_list.append(curr_global_rot_mat[start_t_idx:]) 
        ...
        # human_rot_list = torch.stack(human_rot_list)[0] # T X 3 X 3 
        # human_jnts_list = torch.stack(human_jnts_list)[0] # T X 22 X 3 
        # human_verts_list = torch.stack(human_verts_list)[0] # T X Nv X 3 
        # human_faces_list = np.asarray(human_faces_list)[0] # Nf X 3 
        human_markers_list = torch.stack(human_markers_list)[0] # T X 77 X 3 
        ...
        return human_markers_list, obj_verts_list, actual_len_list
    ...
    def gen_vis_res(self, all_res_list, data_dict, step, vis_gt=False, vis_tag=None, \
        ...
        for idx in range(num_seq):
            ...
            # seq_name_all = '_'.join(curr_seq_name.split('_')[:-1])
            # motion_path = os.path.join(self.ds.data_root_folder, dataset, "sequences_canonical")
            # mesh_verts, mesh_faces, mesh_jnts, angles, root_trans = markers2smpl(dataset,motion_path,seq_name_all,curr_global_markers)
            # np_angles = angles.detach().cpu().numpy()
            # curr_global_rot_mat = torch.from_numpy(Rotation.from_rotvec(np_angles).as_matrix()).float()
