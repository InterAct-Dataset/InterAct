...
######################################## Visualize SMPL ########################################
def visualize_smpl(name, MOTION_PATH, model_type, num_betas):
    """
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        for name in data_name:
            ...
            poses = body_params['global_orient']
            trans = body_params['transl']
            ...
            with np.load(motion_file, allow_pickle=True) as f:
                f = dict(f)
                body = f['body'].item()
                object_ = f['object'].item()
                body_params = body['params']
                object_params = object_['params']
                body_params['global_orient'] = np.array(new_poses)
                body_params['transl'] = np.array(new_trans)
                object_params['global_orient'] = np.array(new_obj_angles)
                object_params['transl'] = np.array(new_obj_trans)
                body['params'] = body_params
                object_['params'] = object_params
                f['body'] = body
                f['object'] = object_

            os.makedirs(os.path.join(NEW_MOTION_PATH, name), exist_ok=True)
            np.savez(os.path.join(NEW_MOTION_PATH, name ,'motion.npz'), **f)
